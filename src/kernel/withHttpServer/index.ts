...
export function withHttpServer(server: HttpServer | WsServer) {
    ...
    server.flows.preRecvDataFlow.push(async (node) => {
        ...
        if (!serviceId) {
            ...
            const urlPath = conn.httpReq.url || ''

            if (urlPath.startsWith('favicon.ico')) {
                // png
            ...
            const handleResults = customRouter.recognize(urlPath)
            const handleResult = first(handleResults)

            if (handleResult) {
                const handler = handleResult.handler as ApiHandle
                const params = handleResult.params

                conn.httpReq.query = handleResults?.queryParams || {}
                conn.httpReq.rawQuery = urlPath.split('?')[1] || ''

                const contentTypeStr = get(conn.httpReq.headers, 'content-type')
                ...
                try {
                    await handler(conn.httpReq, conn.httpRes)
                } catch (e: any) {
                    server.logger.error(e)
                    conn.httpRes.send(e.message || 'Server error', 500)
                }
                return null
            }
